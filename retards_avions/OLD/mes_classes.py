...
class Vols():
    ...
    def get_vols(self, ville_origine = 'Dallas, TX',
        ...
        if 'airline_id' in data_recherche.keys():
            airline = data_recherche['airline_id']
            if airline!=None:
                m_airline_id = (subset['AIRLINE_ID'] == airline)
            else:
                m_airline_id = np.full(subset['AIRLINE_ID'].shape, True, dtype=bool)
        else:
            m_airline_id = np.full(subset['AIRLINE_ID'].shape, True, dtype=bool)

        if 'heure_dep' in data_recherche.keys():
            heure_dep = data_recherche['heure_dep']
            if heure_dep!=None:
                m_heure_dep = (subset['CRS_DEP_TIME'] == heure_dep)
            else:
                m_heure_dep = np.full(subset['CRS_DEP_TIME'].shape, True, dtype=bool)
        else:
            m_heure_dep = np.full(subset['CRS_DEP_TIME'].shape, True, dtype=bool)

        if 'heure_arr' in data_recherche.keys():
            heure_arr = data_recherche['heure_arr']
            if heure_arr!=None:
                m_heure_arr = (subset['CRS_ARR_TIME'] == heure_arr)
            else:
                m_heure_arr = np.full(subset['CRS_ARR_TIME'].shape, True, dtype=bool)
        else:
            m_heure_arr = np.full(subset['CRS_ARR_TIME'].shape, True, dtype=bool)
        
        ## On applique le filtre
        subset = subset[m_airline_id & m_heure_dep & m_heure_arr]
    ...
    ## Methodes pour la gestion des vols
    def get_vols_OLD(self, ville_origine = 'Dallas, TX',
                 ville_arrivee = 'Chicago, IL',
                 colums_sortie = ['ORIGIN_AIRPORT_SEQ_ID', 'DEST_AIRPORT_SEQ_ID', 
                                  'FL_DATE', 'AIRLINE_ID', 'ARR_HOUR', 'DEP_HOUR'],
                 data_recherche = {'date_depart': None, 'airline_id': None, 
...
def get_prediction(self, ville_origine, ville_arrivee, datedep,
                   airline_id, dep_hour, arr_hour):
    l1 = self.get_vols(ville_origine = ville_origine,
                       ville_arrivee = ville_arrivee,
                       data_recherche = {'date_depart': datedep, 
                                         'heure_dep': int(float(dep_hour)), 
                                         'heure_arr': int(float(arr_hour)),
                                         'airline_id': int(float(airline_id))
                                        },
    ...
    idx = m_data[
        (m_data.ORIGIN_AIRPORT_SEQ_ID==l1.ORIGIN_AIRPORT_SEQ_ID.get_values()[0]) &
        (m_data.DEST_AIRPORT_SEQ_ID==l1.DEST_AIRPORT_SEQ_ID.get_values()[0]) &
        (m_data.FL_DATE==l1.FL_DATE.get_values()[0]) &
        (m_data.AIRLINE_ID==l1.AIRLINE_ID.get_values()[0]) &
        (m_data.CRS_DEP_TIME==l1.CRS_DEP_TIME.get_values()[0]) &
        (m_data.CRS_ARR_TIME==l1.CRS_ARR_TIME.get_values()[0])
    ].index.values
